...
draw_circle_width = function(inner_radius, width, segment, percentage) {
	...
	for (var j = 0; j <= 360 * percentage; j += jadd) {
		draw_vertex(
			room_width / 2 - lengthdir_x(radius, j),
			room_height / 2 + lengthdir_y(radius, j)
		);

		draw_vertex(
			room_width / 2 - lengthdir_x(radius + thickness, j),
			room_height / 2 + lengthdir_y(radius + thickness, j)
		);
	}
	draw_primitive_end();
};
