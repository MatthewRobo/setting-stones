...
for (var i = 0; i < sides; i++) {
	...
	var _color = merge_color(color_front, nGradientX > 0 ? color_up : color_down, abs(nGradientX));
	_color = merge_color(_color, nGradientY > 0 ? color_right : color_left, abs(nGradientY));
...
for (var i = 0; i < sides; i++) {
	var j = i < (sides - 1) ? i + 1 : 0;
	draw_vertex(x + lengthdir_x(dists_in[i], angles[i] + rotation), y + lengthdir_y(dists_in[i], angles[i] + rotation));
	draw_vertex(x, y);
	draw_vertex(x + lengthdir_x(dists_in[j], angles[j] + rotation), y + lengthdir_y(dists_in[j], angles[j] + rotation));
